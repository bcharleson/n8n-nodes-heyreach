...
class CampaignOperations {
    ...
    static async get(context, itemIndex) {
        ...
        const response = await generic_functions_1.heyReachApiRequest.call(context, 'GET', `/campaign/GetById?campaignId=${campaignId}`);
        // The GetById endpoint returns campaign data directly in response.data
        // (not wrapped in an items array like GetAll does)
        if (response && typeof response === 'object') {
            return response;
        }
        throw new n8n_workflow_1.NodeOperationError(context.getNode(), `Campaign with ID ${campaignId} not found`, { itemIndex });
    }
    /**
    ...
    static async getMany(context, itemIndex) {
        ...
        if (returnAll) {
            const campaigns = await generic_functions_1.heyReachApiRequestAllItems.call(context, 'POST', '/campaign/GetAll', body);
            return campaigns;
        }
        else {
            const response = await generic_functions_1.heyReachApiRequest.call(context, 'POST', '/campaign/GetAll', body);
    ...
    static async pause(context, itemIndex) {
        const campaignId = this.getCampaignId(context, itemIndex);
        try {
            const response = await generic_functions_1.heyReachApiRequest.call(context, 'POST', `/campaign/Pause?campaignId=${campaignId}`);
    ...
    static async resume(context, itemIndex) {
        const campaignId = this.getCampaignId(context, itemIndex);
        try {
            const response = await generic_functions_1.heyReachApiRequest.call(context, 'POST', `/campaign/Resume?campaignId=${campaignId}`);
            // Check if response has the expected structure
            if (response && typeof response === 'object') {
                // If response has items array, use first item
                if (response.items && Array.isArray(response.items) && response.items.length > 0) {
            ...
            // Fallback: return basic campaign info
            return { id: campaignId, status: 'IN_PROGRESS' };
        }
        catch (error) {
    ...
    static async addLeads(context, itemIndex) {
        ...
        (0, generic_functions_1.validateCampaignStatus)(campaign, 'add leads');
        let accountLeadPairs = [];
        if (leadsInputMode === 'single') {
            ...
            try {
                ...
                for (const leadData of leadsData) {
                    ...
                    const accountLeadPair = { lead: leadData };
        ...
        if (accountLeadPairs.length === 0) {
            throw new n8n_workflow_1.NodeOperationError(context.getNode(), 'No valid leads to add to campaign', { itemIndex });
        }
        // Add leads to campaign using V2 endpoint for better response data
        const response = await generic_functions_1.heyReachApiRequest.call(context, 'POST', '/campaign/AddLeadsToCampaignV2', {
            campaignId,
            accountLeadPairs,
        });
        return {
...
exports.CampaignOperations = CampaignOperations;
//# sourceMappingURL=CampaignOperations.js.map