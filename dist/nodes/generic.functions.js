...
exports.heyReachApiRequest = heyReachApiRequest;
exports.heyReachApiRequestAllItems = heyReachApiRequestAllItems;
exports.validateCampaignStatus = validateCampaignStatus;
exports.validateLinkedInProfileUrl = validateLinkedInProfileUrl;
exports.getLinkedInUrlValidationError = getLinkedInUrlValidationError;
exports.validateCustomFieldName = validateCustomFieldName;
exports.formatDateForApi = formatDateForApi;
exports.validateAndParseId = validateAndParseId;
exports.validateDateRange = validateDateRange;
exports.parseAndValidateIds = parseAndValidateIds;
exports.sanitizeSearchKeyword = sanitizeSearchKeyword;
exports.validatePaginationParams = validatePaginationParams;
const n8n_workflow_1 = require("n8n-workflow");
/**
 * Make an API request to HeyReach
 */
async function heyReachApiRequest(method, endpoint, body = {}, qs = {}) {
 ...
 */
async function heyReachApiRequestAllItems(method, endpoint, body = {}, qs = {}) {
    const returnData = [];
    let responseData;
    ...
    do {
        ...
        if (responseData.items && Array.isArray(responseData.items)) {
            ...
            if (responseData.items.length < body.limit ||
 ...
 */
function validateCampaignStatus(campaign, operation) {
    if (!campaign) {
        throw new n8n_workflow_1.NodeOperationError({}, 'Campaign not found. Please check the campaign ID.');
    }
    const validStatuses = ['IN_PROGRESS', 'PAUSED'];
    if (!validStatuses.includes(campaign.status)) {
        throw new n8n_workflow_1.NodeOperationError({}, `Cannot ${operation} for campaign with status "${campaign.status}". Campaign must be IN_PROGRESS or PAUSED. Current status: ${campaign.status}`);
    }
    if (!campaign.campaignAccountIds || campaign.campaignAccountIds.length === 0) {
        throw new n8n_workflow_1.NodeOperationError({}, `Cannot ${operation} for campaign "${campaign.name}". Campaign must have LinkedIn accounts assigned.`);
    }
}
/**
 ...
 */
function getLinkedInUrlValidationError(url) {
    if (!url || typeof url !== 'string' || url.trim() === '') {
 ...
 */
function validateCustomFieldName(name) {
 ...
 */
function validateAndParseId(id, fieldName) {
    if (typeof id === 'number') {
        return id;
    }
    if (typeof id === 'string') {
 ...
 */
function parseAndValidateIds(idsString, fieldName) {
 ...
 */
function validatePaginationParams(limit, returnAll) {
